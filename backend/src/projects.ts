...
router.get("/all-lite", async (_req, res) => {
  ...
  const rows = await db.all(
    "SELECT id, name, annualPercent, startDate, investedAmount, rateType, marketSymbol FROM projects ORDER BY createdAt DESC"
  );
...
router.get("/summary", async (req, res) => {
  ...
  // Total investment
  const totalInvestmentRow = await db.get(
    "SELECT COALESCE(SUM(investedAmount), 0) as total FROM projects"
  );
  const totalInvestment = totalInvestmentRow?.total ?? 0;

  // Average percent
  const avgPercentRow = await db.get(
    "SELECT COALESCE(AVG(annualPercent), 0) as avg FROM projects"
  );
  const averagePercent = avgPercentRow?.avg ?? 0;

  // Active investments (sum where startDate <= today)
  const activeInvestmentRow = await db.get(
    'SELECT COALESCE(SUM(investedAmount), 0) as total FROM projects WHERE startDate <= date("now")'
  );
...
router.post("/", async (req, res) => {
  ...
  const db = await openDb();
  await db.run(
    "INSERT INTO projects (id, name, annualPercent, startDate, createdAt, investedAmount, rateType, marketSymbol) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    [
      id,
      name,
      normalizedRateType === RateType.Fixed ? annualPercent : null,
      startDate,
      createdAt,
      investedAmount,
      normalizedRateType,
...
router.put("/:id", async (req, res) => {
  ...
  const result = await db.run(
    "UPDATE projects SET name = ?, annualPercent = ?, startDate = ?, createdAt = ?, investedAmount = ?, rateType = ?, marketSymbol = ? WHERE id = ?",
    [
      name,
      normalizedRateType === RateType.Fixed ? annualPercent : null,
      startDate,
